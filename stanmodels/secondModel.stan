...
model {
  ...
  target += gamma_lpdf(sigma | shapes[1], rates[1]);
  target += gamma_lpdf(sd_1  | shapes[2], rates[2]);
  target += gamma_lpdf(sd_2  | shapes[3], rates[3]);
  target += gamma_lpdf(sd_3  | shapes[4], rates[4]);
